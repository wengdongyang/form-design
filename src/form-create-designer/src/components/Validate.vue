<template>
  <div class="_fd-validate">
        ...
        <el-row>
          ...
          <el-col :span="getSpan(item)">
            <el-form-item :label="modes[item.mode]">
              ...
              <template v-else-if="item.mode === 'validator'">
                <FnInput v-model="item[item.mode]" name="name" :args="['rule', 'value', 'callback']" @change="onInput">{{ t('validate.modes.validator') }} </FnInput>
              </template>
              <template v-else>
          ...
          <el-col :span="24">
            <el-form-item :label="t('validate.message')">
              <el-input v-model="item.message" :placeholder="t('validate.requiredPlaceholder')" @blur="onInput">
                <template #append>
                  <span class="append-msg" @click="autoMessage(item)"> {{ t('validate.auto') }} </span>
                </template>
              </el-input>
            </el-form-item>
          </el-col>
    ...
    <el-dropdown trigger="click" size="default" popper-class="_fd-validate-pop" @command="handleCommand" v-if="false">
