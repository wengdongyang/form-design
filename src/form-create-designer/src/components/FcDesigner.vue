      ...
      <el-container style="height: 100%" :key="locale && locale.name">
        <el-aside class="_fc-l" width="266px">
          <el-container style="height: 100%">
            <el-header height="40px" class="_fc-l-tabs">
              <div class="_fc-l-tab" :class="{ active: activeMenuTab === 'menu' }" @click="activeMenuTab = 'menu'">{{ t('menu.component') }}</div>
              <div class="_fc-l-tab" :class="{ active: activeMenuTab === 'tree' }" @click="activeMenuTab = 'tree'">{{ t('menu.tree') }}</div>
            </el-header>
            <el-main v-show="activeMenuTab === 'menu'">
              <template v-for="(item, index) in menuList">
                <div class="_fc-l-group" :key="index" v-if="hiddenMenu.indexOf(item.name) === -1">
                  <h4 class="_fc-l-title" @click="$set(item, 'hidden', !item.hidden)">
                    {{ t('menu.' + item.name) || item.title }}
                    <i class="fc-icon icon-arrow" :class="{ down: !item.hidden }" />
                  </h4>
                  <fcDraggable :group="{ name: 'default', pull: 'clone', put: false }" :sort="false" itemKey="name" class="_fc-l-list" :list="item.list" v-show="!item.hidden">
                    <template v-for="(element, index) in item.list">
                      <div class="_fc-l-item" v-if="hiddenItem.indexOf(element.name) === -1" :key="index" @click="clickMenu(element)">
                        <div class="_fc-l-icon">
                          <i class="fc-icon" :class="element.icon || 'icon-input'"></i>
                        </div>
                        <span class="_fc-l-name">{{ t('com.' + element.name + '.name') || element.label }}</span>
                      </div>
                    </template>
                  </fcDraggable>
                </div>
            ...
            <el-main v-if="activeMenuTab === 'tree'">
              <el-tree ref="treeRef" :data="treeInfo" default-expand-all :expand-on-click-node="false">
                <template #default="{ node, data }">
                  <div class="_fc-tree-node" :class="{ active: activeRule === data.rule }" @click="treeChange(data)">
                    <div class="_fc-tree-label">
                      <i class="fc-icon" :class="(data.rule._menu && data.rule._menu.icon) || 'icon-cell'"></i>
                      <span>{{
                        (data.rule.title || '').trim() ||
                    ...
                    <div class="_fc-tree-more" @click.stop v-if="!data.slot">
                      <el-dropdown trigger="click" size="default" @command="k => toolHandle(data.rule, k)">
                        <i class="fc-icon icon-more"></i>
                        <template #dropdown>
                          <el-dropdown-menu>
                            <el-dropdown-item v-if="data.rule._fc_drag_tag !== '_'" key="1" command="copy">
                              {{ t('props.copy') }}
                            </el-dropdown-item>
                            <el-dropdown-item command="addChild" v-if="data.rule._menu && data.rule._menu.children && data.rule._fc_drag_tag !== '_'" key="2">
                              {{ t('form.appendChild') }}
                            </el-dropdown-item>
                            <el-dropdown-item command="delete" key="3">
                              {{ t('props.delete') }}
                            </el-dropdown-item>
                          </el-dropdown-menu>
                        </template>
          ...
          <el-header class="_fc-m-tools" height="45">
            <div class="_fc-m-tools-l">
              <template v-if="!inputForm.state">
                <template v-if="getConfig('showDevice') !== false">
                  <div class="devices">
                    <i class="fc-icon icon-pc2" :class="{ active: device === 'pc' }" @click="device = 'pc'"></i>
                    <i class="fc-icon icon-pad2" :class="{ active: device === 'pad' }" @click="device = 'pad'"></i>
                    <i class="fc-icon icon-mobile2" :class="{ active: device === 'mobile' }" @click="device = 'mobile'"></i>
                  </div>
                  <div class="line"></div>
                </template>
                <div>
                  <i class="fc-icon icon-pre-step" :class="{ disabled: !operation.list[operation.idx - 1] }" @click="prevOperationRecord"></i>
                  <i class="fc-icon icon-next-step" :class="{ disabled: !operation.list[operation.idx + 1] }" @click="nextOperationRecord"></i>
                </div>
              </template>
            </div>
            <div class="_fc-m-tools-r">
              <template v-if="!inputForm.state">
                ...
                <el-button v-if="getConfig('showSaveBtn', false)" type="success" plain size="mini" @click="handleSave"
                  ><i class="fc-icon icon-save-online"></i> {{ t('props.save') }}
                </el-button>
                <el-button type="primary" plain size="mini" @click="openPreview"><i class="fc-icon icon-preview"></i> {{ t('props.preview') }} </el-button>
                <el-popconfirm
                ...
                >
                  <template #reference>
                    <el-button type="danger" plain size="mini"><i class="fc-icon icon-delete"></i>{{ t('props.clear') }} </el-button>
                  </template>
                </el-popconfirm>
              ...
              <template v-if="getConfig('showInputData', true)">
                ...
                <div class="_fd-input-btn">
                  ...
                  <el-switch size="mini" :value="inputForm.state" inline-prompt @input="openInputData" />
                </div>
              </template>
            </div>
          </el-header>
          <el-main class="_fc-m-con">
            ...
            >
              <div class="_fc-m-input" v-if="inputForm.state">
                <ViewForm :key="inputForm.key" :rule="inputForm.rule" :option="inputForm.option" v-model="inputForm.api" :disabled="false"></ViewForm>
              </div>
              <DragForm v-else :rule="dragForm.rule" :option="formOptions" v-model="dragForm.api"></DragForm>
            </div>
        ...
        <el-aside class="_fc-r" width="320px" v-if="!config || config.showConfig !== false">
          <el-container style="height: 100%">
            ...
            <el-main class="_fc-r-tab-form" v-show="activeTab === 'form'" v-if="!config || config.showFormConfig !== false">
              <DragForm :rule="form.rule" :option="form.option" :value="form.value" @change="formOptChange" v-model="form.api"></DragForm>
            </el-main>
            <el-main class="_fc-r-tab-props" v-show="activeTab === 'props'" :key="activeRule ? activeRule._fc_id : customForm.config ? customForm.key : ''">
              <template v-if="activeRule || (customForm.config && (customForm.config.name || customForm.config.label))">
                <template v-if="false">
                  ...
                  <template v-if="activeRule && activeRule.name">
                    ...
                    <el-input size="mini" class="_fc-r-name-input" :value="activeRule.name" readonly>
                      <template #suffix>
              ...
              <DragForm v-show="baseForm.isShow" v-model="baseForm.api" :rule="baseForm.rule" :option="baseForm.options" :value="baseForm.value" @change="baseChange"></DragForm>
              ...
              ></DragForm>
              <template v-if="false">
                <el-divider v-if="styleForm.isShow">{{ t('designer.style') }} </el-divider>
                <DragForm
        ...
        <el-dialog :visible.sync="preview.state" width="800px" class="_fd-preview-dialog" append-to-body>
          ...
          <template v-if="previewStatus === 'form'">
            <ViewForm :rule="preview.rule" :option="preview.option" v-model="preview.api" v-if="preview.state"></ViewForm>
          </template>
...
import { getRuleTree, getRuleDescription, getFormRuleDescription, upper, useLocale, isNull, formTemplate, uniqueArray, copyTextToClipboard } from '../utils/index';
import viewForm, { designerForm } from '../utils/form';
import { t as globalT } from '../utils/locale';
import EventConfig from './EventConfig.vue';
import { computed, defineComponent, getCurrentInstance, markRaw, nextTick, provide, reactive, ref, toRef, toRefs, watch, set } from 'vue';
  ...
  setup(props, { emit }) {
    ...
    const methods = {
      ...
      openPreview() {
        ...
        data.preview.html = hljs.highlight(formTemplate(rule, options), { language: 'xml' }).value;
      },
      getRule() {
      ...
      parseRule(children, pSlot) {
        return [...children].reduce((initial, rule) => {
          ...
          Object.keys(rule)
            .filter(k => k.indexOf('__') === 0 || (Array.isArray(rule[k]) && rule[k].length === 0) || (is.Object(rule[k]) && Object.keys(rule[k]).length === 0))
            .forEach(k => {
              delete rule[k];
            });
          initial.push(rule);
      ...
      updateRuleFormData() {
        ...
        Object.keys(rule).forEach(k => {
          if (['effect', 'config', 'payload', 'id', 'type', '_menu'].indexOf(k) < 0) formData['formCreate' + upper(k)] = deepCopy(rule[k]);
        });
        Object.keys(rule.props).forEach(k => {
      ...
      makeRule(config, _rule) {
        ...
        if (config.drag) {
          rule.children = [
            (drag = methods.makeDrag(config.drag, rule._menu ? rule._menu.name : rule.type, children, {
